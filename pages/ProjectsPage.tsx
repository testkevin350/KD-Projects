    ...
    <div className="pt-24 md:pt-32 pb-24">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12 md:mb-20">
          <h1 className="text-4xl md:text-8xl font-black mb-4 md:mb-8 tracking-tighter uppercase leading-none">PORTFOLIO</h1>
          <p className="text-copper font-bold tracking-[0.3em] uppercase text-[10px] md:text-sm">Onze Meesterwerken</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
          {projects.map((project, i) => (
            <div key={i} className="group relative overflow-hidden bg-anthracite aspect-[4/5] border border-white/5">
              ...
              />
              <div className="absolute inset-0 p-6 md:p-8 flex flex-col justify-end bg-gradient-to-t from-darkSlate via-transparent to-transparent">
                <span className="text-copper text-[8px] md:text-[10px] font-bold uppercase tracking-widest mb-2">{project.category} — {project.year}</span>
                <h3 className="text-xl md:text-2xl font-bold text-white uppercase tracking-tighter">{project.title}</h3>
